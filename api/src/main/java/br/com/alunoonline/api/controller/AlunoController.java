...
public class AlunoController {
    ...
    @GetMapping("/cep/{cep}")
    public ResponseEntity<String> consultaCep(@PathVariable String cep) {
        log.info("Consultando Cep do aluno: " + cep);
        return ResponseEntity.ok(viaCepClient.consultaCep(cep));
    }

    @GetMapping("/nome/email/{id}")
    public ResponseEntity<AlunoDTO> consutarEmailAlunoByID(@PathVariable Long id) {
        return ResponseEntity.ok(service.consutaAlunoEmail(id));
    }
